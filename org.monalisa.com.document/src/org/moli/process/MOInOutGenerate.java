...
public class MOInOutGenerate extends SvrProcess {
	...
	private MO_InOut 		m_shipment = null;
		...
		MStorageOnHand[] storages, boolean force)
	{
		...
		if (m_shipment == null)
		{
			m_shipment = new MO_InOut (order, 0, m_movementDate);
		...
		for (int i = 0; i < storages.length; i++)
		{
			...
			//      Temp adjustment, actual update happen in MO_InOut.completeIt - just in memory - not saved
	...
	private void completeShipment()
	{
		if (m_shipment != null)
		{
			...
			//reset storage cache as MO_InOut.completeIt will update m_storage
			resetStorageCache();
		}
		m_shipment = null;
		m_line = 0;
