...
public class MOInvoiceGenerate extends SvrProcess
{
		/**	Manual Selection		*/
	...
	/**	The current Invoice	*/
	private MO_Invoice 	m_invoice = null;
	/**	The current Shipment	*/
	private MO_InOut	 	m_ship = null;
	/** Numner of Invoices		*/
	...
	private String generate (PreparedStatement pstmt)
	{
		...
		try
		{
			...
			while (rs.next ())
			{
				...
				MO_Order order = new MO_Order (getCtx(), rs, get_TrxName());
				...
				{
					MO_InOut[] shipments = order.getShipments();
					for (int i = 0; i < shipments.length; i++)
					{
						MO_InOut ship = shipments[i];
						if (!ship.isComplete()		//	ignore incomplete or reversals 
							|| ship.getDocStatus().equals(MO_InOut.DOCSTATUS_Reversed))
							continue;
		...
		BigDecimal qtyInvoiced, BigDecimal qtyEntered)
	{
		if (m_invoice == null)
		{
			...
			m_invoice = new MO_Invoice (order, 0, p_DateInvoiced);
			if (!m_invoice.save())
	 ...
	 */
	private void createLine (MOrder order, MO_InOut ship, MInOutLine sLine)
	{
		if (m_invoice == null)
		{
			try {
