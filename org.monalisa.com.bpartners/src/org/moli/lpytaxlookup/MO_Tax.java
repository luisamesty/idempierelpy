...
//		MLocation lFrom = new MLocation (ctx, billFromC_Location_ID, trxName); 
		MLocation lFrom = new MLocation (ctx, billToC_Location_ID, trxName); 
		...
		log.warning("From Country="+lFrom.getC_Country_ID()+ "To Country="+lFrom.getC_Country_ID());
		for (int i = 0; i < taxes.length; i++)
		{
			MTax tax = taxes[i];
			log.warning("Tax="+tax.getName()+"  tax.isTaxExempt()="+tax.isTaxExempt());
...
//				return getExemptTax(ctx, AD_Org_ID, trxName);
		...
		return get (ctx, C_TaxCategory_ID, IsSOTrx, IsTaxExempt,
			shipDate, shipFromC_Location_ID, shipToC_Location_ID,
			billDate, billFromC_Location_ID, billToC_Location_ID, trxName);
	}	//	getProduct
